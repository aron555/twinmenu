    ...
    public function onAfterCategoryCreate(&$element)
    {
	    if (!empty($element->category_id))
	    {
            ...
            if ($hika_type_menu == "category") { //если выбрана категория, то добавляем для нее параметры
                ...
                $hika_type = '{"hk_category":{"layout_type":"div","columns":"'.$cols_cat.'","rows":"'.$rows_cat.'","limit":"'.$cols_cat*$rows_cat.'","div_item_layout_type":"'.$div_item_layout_type.'","category":"'.$element->category_id.'","show_description":"'.$show_description.'"},"hk_product":{"layout_type":"inherit","columns":"'.$cols_pr.'","rows":"'.$rows_pr.'","limit":"'.$cols_pr*$rows_pr.'","div_item_layout_type":"inherit","filter_type":"2"}}';
